...
%
\begin{abstract}%
Testing is one of the most wide-spread means for quality assurance. Modeling and
...
The Unified Modeling Language\textsuperscript{\texttrademark} (UML) is the quasi
...
implementation. Activity diagrams support hierarchical modeling whereby it is
...
In this section, we introduce the proramming language AMPL and the semantics of
...
% 
% In AMPL models we can declare parameters, sets, variables, and constraints. We
% will give specific values for the parameters and sets in the AMPL data. A
% suitable state--of--the--art solver can assign the variables such that all
% constraints are satisfied with the given values of parameters and sets.
\begin{table}%
\begin{center}%
...
semantics of the expected input model. Whenever we refer to a modeling element
...
font. In this section, we first detail which UML modeling elements we
...
\UMLType{ControlNode}s, and \UMLType{ControlFlow}s modeling the control flow of
...
covers all simple paths in the test model.
...
steer which coverage criteria the generated test cases will adhere to.
...
the modeler originally had in mind. We support the user by automatically adding
...
variable will be modeled in AMPL as an array of variables. Each parameter is
...
are modeled as a single variable in AMPL and we assume $x$ to be a variable that
is modeled as an array of variables. We further assume that the
\UMLReference{type} of the modeling elements corresponding to the OCL variables
