...
\end{algorithm}%
\section{Results}%
The presented algorithm is implemented as a feature of the Eclipse plug-in ParTeG \cite{PartegWebsite}. Further the constraint solver--based approach has also been applied to UML \UMLType{StateMashine}s \cite{Amin:Thesis}. Further we built a set of example models with corresponding implementations to test our implementation. Finally, our tool has also been applied in an industrial case study at Airbus Operations GmbH.\\%
In this section we will first Explain how to use the tool and then summarise the results of the Industrial case study.
\subsection{Proof--of--Concept Tool}%
\label{sec:EclipsePlugIn}%
The tool implementing the presented algorithm is called Activity Test Case Generator. It is integrated into the Partition Test Generator (ParTeG) from Stephan Wei{\ss}leder.
\section{Case Study}%
\label{sec:CaseStudy}
