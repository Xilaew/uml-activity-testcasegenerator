...
Modeling and test automation are two ways to improve effectivity and efficiency of testing.
In this paper, we present a method to generate test data from UML activity diagrams and OCL constraints. 
Our test generation method combines symbolic execution and state--of--the--art constraint solvers.
The generated tests satisfy control--flow--based coverage criteria on the used models. 
To improve performance of test-data generation and the quality of the test data, we introduce infeasible path elimination and boundary value analysis. 
Our prototype implementation is integrated in the existing test generator ParTeG, available as Eclipse plug-in, and generates fully automatically compilable C++ unit tests from activity diagrams. 
We provide an industrial case study and measure the performance of our tool with different solvers in several scenarios. 
