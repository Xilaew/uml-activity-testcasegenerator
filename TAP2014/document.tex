...
\label{sec:AMPL}
We use `A Mathematical Programming Language' (AMPL) by Robert Fourer \cite{AMPL} as means to formalise the execution of a control flow path in the test model. %In this subsection we will first mention the language elements of AMPL that we are using and then introduce the advantage of having AMPL as a common interface to different solvers suitable for distinct specializations of the constraint satisfaction problem.\\%
An AMPL program consists of two parts, the AMPL model and the AMPL data. In AMPL model we can declare parameters, sets, variables and constraints. In the AMPL data we will give specific values for the parameters and sets. A suitable state--of--the--art solver can assign the variables that all constraints are satisfied with the given parameters and sets.
\begin{table}%
\begin{center}%
...
\end{figure}%
An AMPL model encodes a complete activity diagram including all variables, parameters and all OCL constraints that are contained. A control flow path within this activity test case graph is encoded in the AMPL data. The activity diagram in Figure \ref{fig:AssignmentDecision} will serve as an example.\\
We model the execution of an \UMLType{Activity} as a series of states. Each AMPL model has one parameter called \AMPLCode{pathlength} representing the number of \UMLType{Actions} on a control flow path. The state always changes during the execution of an \UMLType{Actions}, consequently, there is one initial state and one post state after each execution of an \UMLType{Action}. The OCL constraints in Figure \ref{fig:AssignmentDecision} reference three different variables. We assume $a$ and $returnValue$ are the \UMLType{Parameter}s of the specifying \UMLType{Operation}. A \UMLType{Parameter}'s value can not change during the execution of an activity diagram, consequently they will be modelled as a single variable in AMPL. We assume $x$ to be a \UMLType{Property} that can change its value, consequently we need to model $x$ as an array of variables in AMPL; one variable for each state. We further assume that the \UMLReference{type} of the modelling elements corresponding to the OCL variables $a$, $returnValue$ and $x$ is integer. Consequently, we generate the following AMPL code for them: %
\begin{lstlisting}[basicstyle=\ttfamily,language=ampl]
param pathlength;
var a : integer >= -1000 ;
var x{0..pathlength} : integer >= 1000;
\end{lstlisting}%
%
During execution a \UMLType{ControlFlow}'s \UMLReference{guard} or an \UMLType{Action}'s \UMLReference{localPostcondition} is always evaluated with respect to the current state. All OCL post--conditions and OCL guards are contained in the AMPL model as constraints, which can be switched on and off for each state.  The set of states in which a \UMLReference{guard} or \UMLReference{localPostcondition} needs to evaluate to true is called activation set.%
%
%
%
...
Every variable has one out of three possible types: integer, real and boolean. In AMPL, by default, a variable is in the domain of real numbers.
Since a variable can have different values in each state of the execution, each variable will be modelled in AMPL as a variable array of fixed size. The size of the array is defined by the \texttt{pathlength}. Parameters are constant and cannot change their value from one state to another we can model those variables as a single AMPL variable.
For each TCGAction the activation set is declared as a subset of $\left[0..\texttt{pathlength}\right]$ in the AMPL model. Each local post--condition of an action is transformed into an indexed collection of constraints over the activation set.\\
