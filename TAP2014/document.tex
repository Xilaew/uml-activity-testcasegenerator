...
%
\begin{abstract}%
We present a method to generate test data from UML activity diagrams. Our method uses symbolic execution to collect embedded OCL constraints along a control flow, translates those constraints into the modelling language AMPL, and derives test data using state--of--the--art constraint solvers that interface with AMPL; OCL constraints may use mixed integer non--linear programming as well as logical formulas. The generated tests will satisfy control--flow--based coverage criteria on the used models. 
To improve performance of test-data generation and the quality of the test data themselves, we introduce infeasible path elimination and  boundary value analysis. 
The tool is integrated in the Partition Test Generator ParTeG, available as Eclipse plug-in, and generates fully automatically compilable C++ unit tests from activity diagrams. Using an industrial case study we measure the performance of our tool with different solvers and vary  maximum path lengths and the number of test cases. For path lengths greater than 50 infeasible path elimination reduces computation times from days to less than an hour; boundary value analysis adds only negligible overhead. 
...
%An AMPL program consists of two parts, the AMPL model and the AMPL data. 
...
\label{sec:TestModel}%
For the transformation into `A Mathematical Programming Language' (AMPL) presented in Section \ref{sec:AMPLTransformation} we need to clarify the semantics of the expected input model. Whenever we refer to a modelling element such as \UMLType{Activity} or a reference defined in the UML as, for example, \UMLReference{name} or \UMLReference{ownedParameter} we print it in a special font. In this section, we first %clarify 
...
% \State abstractTestCases : Set of path--tree nodes $\gets$ \{\} \Comment initially empty set
 ...
 \While{!stack$\rightarrow$isEmpty() \textbf {and} path tree contains less than MaxNoPaths leaf nodes}
   \State ptn : path--tree node $\gets$ stack$\rightarrow$pop() \Comment remove first path--tree node from stack
   \State f : integer $\gets$ ptNode.target.outgoing$\rightarrow$size() \Comment fanout of current node
     \If{f$>$0 \textbf{and} ptn.depth$<$MaxPathLen \textbf{and} (decisions$<$UchkSteps \textbf{or} f$\geq$2 \textbf{or} ptn$\rightarrow$isFeasiblePath())} \Comment evaluate isFeasiblePath() only if its value is relevant
       \For{cf : \UMLType{ControlFlow} $\in$ ptNode.controlFlow.target.outgoing}
         \State stack$\rightarrow$push( \textbf{new} path--tree node(cf, ptn.depth+1, ptn) ) 
       \EndFor
...
The model was provided as XMI export from Atego\textsuperscript{\textregistered} Artisan Studio. Each modelling tool uses its own implementation of the UML meta model. Due to slight differences in the implementations it is not directly %simply 
