...
public class TestCaseGraphOutputJavaPrioritizationAnalysis extends TestCaseGraphOutputJava {

	public TestCaseGraphOutputJavaPrioritizationAnalysis(TestCaseGraphHelper in_oTCGHelper)
	{
		super(in_oTCGHelper);
	}
	...
	public void createGlobalEnd(TestSuite in_oTestSuite) {
		...
		m_oFormattedOutput.openContext();
		m_oFormattedOutput.append("m_colTestCaseToFirstMutantKills.add(0);");
		...
		for(int i = 0; i < m_nTestMethodCounter; ++i) {
			...
			m_oFormattedOutput.appendln("m_colTestCaseToAbsoluteMutantKills.set(" + (i) + ", "
		...
		m_oFormattedOutput.appendln("System.getProperty(\"line.separator\"));");
		m_oFormattedOutput.closeContext();
		...
		m_oFormattedOutput.appendln("System.out.println(\"mutants of test case \" + (i+1) + \": \""
				...
				+ ");");
		m_oFormattedOutput.appendln("nTestRunsToFirstDetection += (double)m_colTestCaseToFirstMutantKills.get(i) * (i+1);");
		m_oFormattedOutput.closeContext();
