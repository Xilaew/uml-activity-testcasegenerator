...
\end{center}
\caption{Screenshot of a valid model in the model browser of Atego\textsuperscript{\textregistered} Artisan Studio}
...
\end{figure}
An abstract test case graph is a directed graph consisting of nodes (\TCGType{AbstractTCGNode}) and edges (\TCGType{AbstractTCGEdge}). Each edge has a \TCGType{source} and a \TCGType{target} node. Each node can have multiple \TCGType{outgoing} and multiple \TCGType{incoming} edges.
A node can be either a pseudo node (\TCGType{AbstractTCGPseudoNode}) or a real node (\TCGType{AbstractTCGRealNode}). A pseudo node is a node that can be inserted in the middle of an edge without changing the semantics of the test model. Edges can have a \TCGType{guard} condition of type \TCGType{AbstractTCGConstraint}. Such a graph is contained by a container (\TCGType{AbstractTCGContainer}). The container has a singular \TCGType{initialNode} reference to one of its \TCGType{nodes} exposing this as the initial node. The class diagram of the abstract test case graph meta model is shown in Figure \ref{fig:AbstractTCGMetaModel}.\\
...
The \TCGType{TCGActivity} is an extension of the \TCGType{AbstractTCGContainer}. \TCGType{TCGAction}, \TCGType{TCGControl-Node}, and \TCGType{TCGControlFlow} refine the types \TCGType{AbstractTCGRealNode}, \TCGType{AbstractTCGPseudo-Node}, and \TCGType{AbstractTCGEdge} respectively. The main extensions to the abstract test case graph meta model are the variables (\TCGType{TCGVariable}), the elements of an abstract syntax tree (\TCGType{TCGOCLExpression}) to express constraints, and the \TCGType{TCGAction}.
\paragraph{TCGAction} A \TCGType{TCGAction} can, in addition to its super--type, contain arbitrary many \TCGType{localPostconditions} of the type \TCGType{AbstractTCGConstraint}. A local post--condition has the semantics that it has to be true after the execution of the action. This implies potentially changing some variables throughout the execution of the action.
\paragraph{TCGOCLExpression}
\TCGType{TCGOCLExpression} is a subtype of \TCGType{AbstractTCGConstraint}. A \TCGType{TCGOCLExpression} can either be a \TCGType{TCGOCLOperation} call with a \TCGType{source} and \TCGType{arguments}, or a \TCGType{TCGOCLLiteralExpression} holding a literal value and its data type, or a \TCGType{TCGOCLVariableCallExp} referencing a \TCGType{TCGVariable}. The isPre attribute of \TCGType{TCGOCL-VariableCallExp} denotes whether the original OCL expression contained an \verb=@pre= token. \TCGType{TCGOCLExpression} and its subtypes form a simplified OCL abstract syntax tree.
\paragraph{TCGVariable}
A \TCGType{TCGVariable} can be one of two subtypes: either a \TCGType{TCGBasicVariable} or a \TCGType{TCGObjectVariable}. An object variable is put together by one or more other variables. A basic variable has a variableType attribute holding a \TCGType{TCGBasicVariableType} literal. The \TCGType{TCGBasicVariableType} literal represents one of the types that our algorithm can handle: integer, real, and boolean. A \TCGType{TCGVariable} is a placeholder for a value. When the isParameter attribute is true it can only hold one value throughout the execution of the \TCGType{TCGActivity}, otherwise it can change its value during each execution of a \TCGType{TCGAction}.
...
\end{figure}
An instance of the activity test case graph meta model serves as a normalised input model. We use a model--to--model transformation to transform an \UMLType{Activity} from a UML model into an activity test case graph. In many cases the transformation is straightforward: for one element in the UML model the corresponding element in the activity test case graph is produced. When one element of the UML model is mapped to one element of the activity test case graph model we call the UML element source element and the created element in the activity test case graph model target element. The straight forward mappings from one source element to one target element are shown in Figure \ref{fig:UML2TCGTranformation}.
\subsubsection{Mapping UML Elements to Activity Test Case Graph Elements}
\paragraph{Activity}
A UML \UMLType{Activity} is transformed into a \TCGType{TCGActivity}. For the C++ unit test synthesis described in Section \ref{sec:testgenerationUnitTestSynthesis} the name of the containing \UMLType{Class} of the source element as well as the full pathname to the \UMLType{Package} containing this \UMLType{Class} need to be stored. The class name and the full pathname will be stored in the property fields of the \TCGType{TCGActivity}. For each \UMLType{Activity} all of its \UMLReference{ownedNodes} as well as \UMLReference{ownedEdges} are considered for transformation.
\paragraph{Action} \UMLReference{OwnedNodes} of an \UMLType{Activity} that are a subtype of \UMLType{Action} are transformed into a \TCGType{TCGAction}. The \UMLType{Action}'s \UMLReference{localPostcondition} reference may contain textual OCL that will be parsed. The detailed semantics of special subtypes of an \UMLType{Action} is neglected.
\paragraph{ControlNode} \UMLReference{OwnedNodes} of the \UMLType{Activity} that are a subtype of \UMLType{ControlNode} are transformed into \TCGType{TCGControlNodes}. That means they are pseudo nodes. It is assumed that there is only one UML \UMLType{InitialNode} whose target element is referenced by the \TCGType{TCGActivity} as \TCGType{initialNode}.
\paragraph{ControlFlow} Based on the \UMLReference{ownedEdges} association of the \UMLType{Activity} we find \UMLType{ControlFlow}s that are transformed into \TCGType{TCGControlFlows}. If there is a \UMLType{ValueSpecification} contained in the \UMLReference{guard}, the potentially contained textual OCL needs to be parsed.
...
\UMLType{Constraint}, \UMLType{ValueSpecification}, and \UMLType{Property} as well as \UMLType{Parameter} can not be transformed straightforwardly. The transformation will be done in three steps. First, textual OCL expressions will be extracted from \UMLType{ValueSpecification}s and then the textual OCL expression will be parsed. When the OCL expression was parsed correctly the parser returns an abstract syntax tree of this expression. In the third step the elements of this abstract syntax tree will be transformed to \TCGType{TCGOCLExpressions} and all \UMLType{Property} and \UMLType{Parameter} elements referenced by the OCL abstract syntax tree will be transformed into \TCGType{TCGVariables}.\\
We ensure that only those OCL expressions are in the activity test case graph that later on can be transformed into an AMPL model as explained in in Section \ref{sec:atcg2Ampl}. There might for example be \UMLReference{ownedAttribute}s of a \UMLType{Class} that are not changed by any of the transformed \UMLType{Actions} and whose value do not have any influence on any of the transformed \UMLType{ControlFlow}'s \UMLReference{guard}. The activity test case graph will not contain any \TCGType{TCGVariable} representing such an irrelevant \UMLType{Property} or \UMLType{Parameter}.
...
<BooleanLiteral>, <IntegerLiteral>, and <RealLiteral> are transformed into a \TCGType{TCGOCLLiteralExp}. Its value is stored in double precision floating point format. The type attribute of the target element is set according to the type of the source element. For the type boolean a value of $0.0$ means false and $1.0$ means a true.\\
<BooleanVariable>, <IntegerVariable>, and <RealVariable> tokens are transformed into \TCGType{TCGOCLVariableCallExp} elements. If the original OCL token was tagged with an \verb=@pre= then the isPre attribute is set to true otherwise it is false. <BooleanVariable>, <IntegerVariable>, and <RealVariable> elements represent an arbitrary OCL expression that evaluates either to a \UMLType{Property} or a \UMLType{Parameter}. The \UMLType{Property} or \UMLType{Parameter} pointed to by the expression will be transformed into a \TCGType{TCGVariable} and the \TCGType{variable} reference of the \TCGType{TCGOCLVariableCallExp} will be set to point to this \TCGType{TCGVariable}.\\
We transform a <LogicalOpSymbol>, <RelationOpSymbol>, or <ArithmeticOpSymbol> into the enumeration literal of the \TCGType{TCGOCLOperationType} enumeration that matches the token represented by the source element. The token \verb§=§ matches for example the equals enumeration literal.\\
Each <LogicalOperation>, <ArithmeticOperation>, and <RelationalOperation> will be transformed into a \TCGType{TCGOCLOperationCallExp}. The value of the \TCGType{operation} attribute will be obtained by transforming the <RelationalOpSymbol>, <LogicalOpSymbol>, or <ArithmeticOpSymbol> contained in the source element. The target element of the first operand of each operation will be stored in the \TCGType{source} reference of the \TCGType{TCGOCLOperation}. The target element of the second operand will be stored in the \TCGType{arguments} reference of the \TCGType{TCGOCLOperationCallExp}.\\
The productions <Bool> and <Number> in Algorithm \ref{alg:OCLEBNF} are there to keep the EBNF notation more readable. No \TCGType{TCGOCLExpression} element is generated for them but the transformation of a <Bool> or <Number> element will transform the contained abstract syntax tree node and return the result of this transformation.\\
Every root \TCGType{TCGOCLExpression} directly contained by a \TCGType{TCGAction}, \TCGType{TCGControlFlow}, or \TCGType{TCGActivity} gets its \TCGType{name} property set. The transformation ensures that every root \TCGType{TCGOCLConstraint} has a globally unique name. In the original UML model it is not the case that each \UMLType{Constraint} or \UMLType{ValueSpecification} has a globally unique \UMLReference{name}.
\paragraph{Transforming Properties and Parameters}
UML \UMLType{Property} or \UMLType{Parameter} elements referenced by a <BooleanVariable>, <IntegerVariable>, or <RealVariable> element will be transformed into a \TCGType{TCGVariable}. All \TCGType{TCGVariables} are contained by the \TCGType{variables} reference of the root element of the activity graph, the \TCGType{TCGActivity}.\\
For \UMLType{Parameter} and \UMLType{Property} elements we need to check their \UMLReference{type} reference. If the \UMLReference{name} of the \UMLType{Type} is one out of a list of names that can be mapped to a literal of the \TCGType{TCGBasicVariableType} enumeration then a \TCGType{TCGBasicVariable} is created for it and its \TCGType{variableType} field set accordingly to the enumeration literal that is either integer, real, or boolean. The exact mapping of \UMLReference{name}s of the \UMLType{Type} to a \TCGType{TCGvariableType} is implementation specific. One could for example map from a \UMLType{Type} named `uint32{\_}t' to integer.\\
If a \UMLType{Parameter} was transformed into a \TCGType{TCGBasicVariable} then the \TCGType{isParameter} field is set to true, indicating that this variable has been a \UMLType{Parameter}. For every \UMLType{Property} the \TCGType{isParameter} field of the target element will be set to false.\\
The field \TCGType{usage} of the \TCGType{TCGBasicVariable} can take values from the enumeration \TCGType{TCGVariableUsage}. This information will be needed when creating the C or C++ unit tests as described in Section \ref{sec:testgenerationUnitTestSynthesis}. If the \TCGType{TCGBasicVariable} was created to represent a \UMLType{Parameter} then its \TCGType{usage} can be \TCGType{inParameter}, \TCGType{outParameter}, \TCGType{inoutParameter}, or \TCGType{returnParameter}. The value of the \TCGType{usage} field for a transformed \UMLType{Parameter} is determined by the value of the \UMLReference{direction} field of the original \UMLType{Parameter}. If the source element of a \TCGType{TCGBasicVariable} was a \UMLType{Property} then its \TCGType{usage} is \TCGType{field}.
...
\end{algorithm}
When a solver successfully computed an answer for a given constraint satisfaction problem we read the assigned values and store them in a test case model. In Algorithm \ref{alg:TestCaseModelGen} we see the pseudo code of the algorithm creating a \TCGType{TestCase} object from the values computed by the constraint solver. We assume that the function AMPL(`x') returns the content of the AMPL variable or variable array x as array. The function createValue(var : \TCGType{TCGVariable}) creates a new \TCGType{Value} object with its name attribute set to var.name and its variable reference pointing to var.\\
The root element of a test case model is a \TCGType{TestSuite}. We will create one \TCGType{TestSuite} element for a \TCGType{TCGActivity}. For each abstract test case on that \TCGType{TCGActivity} that has been successfully solved we will create one \TCGType{TestCase} and add it to the \TCGType{TestSuite}. For each \TCGType{TCGVariable} with its isParameter attribute set to false in the \TCGType{TCGActivity} we will retrieve an array of values from the AMPL process. The first value in this array is stored in a \TCGType{Value} element. The name of this \TCGType{Value} element is set to the name of the \TCGType{TCGVariable} and it is added to the \TCGType{initValues} reference of the \TCGType{TestCase}. In the same way, the last element from the array retrieved from AMPL will be stored in a \TCGType{Value} element and stored in the \TCGType{testForValue} reference. A \TCGType{FunctionCall} with the same name as the \TCGType{TCGActivity} will be created. For each \TCGType{TCGVariable} with its isParameter attribute set to true a single value will be retrieved from AMPL, and, depending on the usage property of the \TCGType{TCGVariable}, the produced \TCGType{Value} element will be stored either in the \TCGType{FunctionCall}'s \TCGType{parameters} or the \TCGType{TestCase}'s \TCGType{testForValue} reference. For out parameters and return parameters the \TCGType{Value} elements are stored in the \TCGType{testForValue} set. For in parameters the \TCGType{Value} element is stored in the \TCGType{FunctionCall}'s \TCGType{parameters}.
