...
A \emph{control flow path} is represented as a sequence of control flows. An \emph{abstract test case} is a control flow path starting at the initial node of the activity and ending at a \emph{final node}. Any node with no outgoing control flow is a final node. For two subsequent control flows, a and b, in a control flow path the OCL expression $a.target = b.source$ is true. \\
