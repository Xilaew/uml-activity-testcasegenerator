...
The semantic of the activity test case graph is like a Petri net. When executing a test case graph we have at the beginning a token in the initial node that can move along the enabled edges. An edge is enalbed when its guard condition is true. We say that a node is being executed when a token resides in a the node.
...
TCGOCLExpression is a subtype of AbstractTCGConstraint. A TCGOCLExpression can either be a TCGOCLOperation call with a source and arguments, or a TCGOCLLiteralExpression holding a literal value and its data type, or a TCGOCLVariableCallExp referencing a TCGVariable. The isPre attribute of TCGOCL-VariableCallExp denotes whether the original OCL expression contained an `@pre' token. TCGOCLExpression and its subtypes form a simplified OCL abstract syntax tree.
