...
\abstract{Testing is one important way of validating and verifying the behaviour of software artefacts. With the use of Model--Based Engineering comes the need to ensure that the implementation derived from models actually behaves as specified in the corresponding models. \\
% Who is 'we'? / how about '..unit tests are generated..'?
In this thesis, we generate unit tests from UML activity diagrams modelling C components.  We use symbolic execution to transform embedded OCL constraints along a control flow path into a rigorous mathematical program expressed in ``\textbf{A} \textbf{M}athematical \textbf{P}rogramming \textbf{L}anguage'' (AMPL) and derive test data by having it solved with state--of--the--art constraint solvers.
% each control flow path to test vity diagram with . 
The generated tests comply with control flow based coverage criteria. Since unit tests using test data at the bounds of path constraints do have a higher probability to detect faults, we also use boundary value analysis for testing. A special focus is on allowing mixed integer non--linear programming as well as logical formulas in OCL constraints, since we want to put as little restrictions as possible on the used test models.}
%So what's the result? Did you solve some problem, does your solution work better than someone else's?
\pagenumbering{Roman}
\tableofcontents
\clearpage
