...
We use symbolic execution to transform embedded OCL constraints along a control flow path into a rigorous mathematical program expressed in `\textbf{A} \textbf{M}athematical \textbf{P}rogramming \textbf{L}anguage' (AMPL) and derive test data by having it solved with state--of--the--art constraint solvers.
% each control flow path to test vity diagram with . 
The generated tests comply to control flow based coverage criteria. Since unit tests using test data at the bounds of path constraints do have a higher probability to detect faults, we also use boundary value analysis for testing. A special focus is on allowing mixed integer non--linear programming as well as logical formulas in OCL constraints, since we want to put as little restrictions as possible on the used test models.}
\pagenumbering{Roman}
\tableofcontents
\clearpage
