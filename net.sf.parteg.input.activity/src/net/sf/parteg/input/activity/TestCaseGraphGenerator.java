package net.sf.parteg.input.activity;
...
import net.sf.parteg.base.testcasegraph.helperclasses.UMLActivityNodeHelperClass;
...
public class TestCaseGraphGenerator {
	...
	// helper constructs
	private HashMap<ActivityNode, TCGNode> m_colActivityNode2TCGNode;
	private Integer m_nTmpNodeCounter;
	private HashMap<Event, TCGEvent> m_colEventMapping;

	public TestCaseGraphGenerator(Activity in_oActivity) {
		m_oActivity = in_oActivity;
		m_colActivityNode2TCGNode = new LinkedHashMap<ActivityNode, TCGNode>();
		m_nTmpNodeCounter = 0;
		m_colEventMapping = new LinkedHashMap<Event, TCGEvent>();
	}

	/*
	 * for state machine: copy all states; copy transitions, add additional
	 * nodes and arcs, add events with parameters and add test goals
	 */
	public TestCaseGraphHelper createTestCaseGraph() throws Exception {
		@SuppressWarnings({ "unused" })
		URI oURI = URIConverter.URI_MAP.get(URI
		...
		if (Configuration.getTransformInitialAndFinalStates()) {
			...
			// 1) all initial nodes of substatemachines are converted into
			// connectionpoints
			getTestCaseGraphHelper().replaceInitialNodesWithConnectionPoints();
			// 2) all final states are converted into states with an outgoing
			// transition to a connectionnode
		...
		// after splitting, each decision node has exactly one incoming
		// transition
		...
		// create hashmap from each referenced variable to the element that
		// references it
		getTestCaseGraphHelper().createVariableReferences();

		// getTestCaseGraphHelper().createAttributeToGetterReferences(m_oStateMachine);

		// configure the nodes of the testcasegraph (distance to initial node,
		// etc.)
	...
	// copyAllStatesForRegion(oRegion, m_oTCGHelper.getTestCaseGraph()
	// .getRegions(), in_colStateMachineRegions,
	// new HashMap<StateMachine, TestCaseGraph>());
	//
	// }

	private void copyAllActions(Activity activity/*,
			List<TestCaseRegion> in_colRegions,
			HashMap<Activity, TestCaseGraph> io_colSubactivityMapping*/) {
		// reference all used regions for adding the transitions

		TestCaseRegion oTCRegion = GeneratedFactory.eINSTANCE
				.createTestCaseRegion();
		for (ActivityNode node : activity.getOwnedNodes()) {
			if (node instanceof Action) {
				TCGRealNode oTCGRealNode = m_oTCGHelper.addRealNode(oTCRegion);
				oTCGRealNode.setName(UMLActivityNodeHelperClass
						.getFullName(node));
				m_colActivityNode2TCGNode.put(node, oTCGRealNode);
				oTCGRealNode.setOriginalNode(node); // used for event
													// parameter instances

				// handle state invariants
				// TODO Add Postconditions/Preconditions
				// addStateInvariant((State) oVertex, oTCGRealNode,
				// Configuration
				// .getSplitInequationsBeforeTestGoalDefinition());
				...
				// // was the submachine already copied? Then just reference
				// // it.
				// TestCaseGraph oReferencedTCG = io_colSubactivityMapping
				// .get(oState.getSubmachine());
				...
				// copyAllConnectionPointsAndReferences(oState,
				// oTCGRealNode);
				// } else {
				// oTCGRealNode.setSubGraph(GeneratedFactory.eINSTANCE
				// .createTestCaseGraph());
				// // oTCGNode.getSubGraph().getSuperStates().add(oTCGNode);
				// oTCGRealNode.getSubGraph().setCompositeSuperState(
				// oTCGRealNode);
				// io_colSubactivityMapping.put(oState.getSubmachine(),
				// oTCGRealNode.getSubGraph());
				// copyAllConnectionPoints(oState.getSubmachine(),
				// oTCGRealNode.getSubGraph());
				// copyAllConnectionPointsAndReferences(oState,
				// oTCGRealNode);
				// for (Region oRegion : oState.getSubmachine()
				// .getRegions()) {
				// copyAllStatesForRegion(oRegion, oTCGRealNode
				// .getSubGraph().getRegions(),
				// io_colStateMachineRegions,
				// io_colSubactivityMapping);
				// }
			} else { // oVertex instanceof ControlNode
				ControlNode oControlNode = (ControlNode) node;

				// do not copy pseudo states here ...
				if (!(oControlNode instanceof InitialNode || oControlNode instanceof 
						FinalNode)) {
					TCGIntermediateNode oTCGIntermediateNode = m_oTCGHelper
							.addIntermediateNode(oTCRegion, TCGNodeType.UNKNOWN);
					oTCGIntermediateNode.setName(UMLActivityNodeHelperClass
							.getFullName(oControlNode));
					m_colActivityNode2TCGNode.put(oControlNode,
							oTCGIntermediateNode);

					oTCGIntermediateNode.setOriginalNode(null);
					if (oControlNode instanceof InitialNode) {
						oTCGIntermediateNode.setNodeType(TCGNodeType.INITIAL);
					} else if (oControlNode instanceof DecisionNode
							|| oControlNode instanceof MergeNode) {
	...
	/*
	 * just copy the connection points - the connection point references are
	 * contained in the referencing super state
	 */
...
//			TCGIntermediateNode oNode = GeneratedFactory.eINSTANCE
//					.createTCGIntermediateNode();
...
//			m_colActivityNode2TCGNode.put(oPseudoState, oNode);
//			if (oPseudoState.getKind().equals(
//					PseudostateKind.ENTRY_POINT_LITERAL)) {
//				oNode.setNodeType(TCGNodeType.ENTRY);
//			} else if (oPseudoState.getKind().equals(
...
//	}
	 ...
	 */
//	private void copyAllConnectionPointsAndReferences(State in_oState,
//			TCGRealNode in_oNode) {
//		// first case: composite state - contains connectionpoints and
...
//						if (oTmpPs.getKind().equals(
//								PseudostateKind.ENTRY_POINT_LITERAL)
//								|| oTmpPs.getKind().equals(
//										PseudostateKind.EXIT_POINT_LITERAL)) {
...
//				}
//			}
//			// create connectionpointreferences that reference the
//			// connectionpoints
//			// the following part should be unused - no
//			// connectionpointreferences are used for composite states
//			for (ConnectionPointReference oCPR : in_oState.getConnections()) {
//				TCGIntermediateNode oNode = GeneratedFactory.eINSTANCE
//						.createTCGIntermediateNode();
//				in_oNode.getContainedConnectionPoints().add(oNode);
//				oNode.setContainer(in_oNode);
//				m_colActivityNode2TCGNode.put(oCPR, oNode);
...
//		}
		...
		// second case: submachine state - contains just the
		// connectionpointreferences
...
//	private void connectConnectionPointAndReference(
//			Pseudostate in_oPseudoState,
//			TCGIntermediateNode in_oConnectionPointReference) {
//		TCGIntermediateNode oReferencedNode = (TCGIntermediateNode) m_colActivityNode2TCGNode
//				.get(in_oPseudoState);
//		in_oConnectionPointReference.getReferencedConnectionPoints().add(
//				oReferencedNode);
//		oReferencedNode.getReferencedConnectionPointReferences().add(
//				in_oConnectionPointReference);
//	}
		...
		for (Region oRegion : in_colStateMachineRegions) {
			for (Transition oTransition : oRegion.getTransitions()) {
				...
				if (Configuration.getEnforceUnambiguousGuardsAndEvents()) {
					...
					List<Transition> colInterferingTransitionsWithTheSameTrigger = new ArrayList<Transition>();
					// if(Configuration.getEnforceUnambiguousGuards()) {
					// check guards of outgoing transitions of the same source
					// node

					// just remove conflicts with super transitions statically -
					// look at the paper!
					...
					// String sGuard =
					// SystemModelHelper.getTransitionGuard(oInterferingTransition);
					// if(!sGuard.equals("else"))
					// colInterferingTransitionsWithTheSameTrigger.add(oInterferingTransition);
					// }
					// }
					boolean bEventsExisting = false;
					// check super transitions only if this transition is not
					// default-triggered
					if (!colEvents.isEmpty()) {
						bEventsExisting = true;
						// if: outer transition has priority over inner
						// transition
						// then: modify inner transitions
						// else: decide separately within the test generation
						// process
						if (!Configuration
								.isInnerTransitionHasPriorityOverOuterTransition()) {
							// check guards of outgoing transitions of super
							// states
							colInterferingTransitionsWithTheSameTrigger
									.addAll(TCGTransitionHelperClass
											.getCompositeStateSuperTransitionsTriggeredByTheSameEvent(oTransition));
						}
						// else {
						// // check guards of outgoing transitions of super
						// states
					...
					for (Transition oInterferingTransition : colInterferingTransitionsWithTheSameTrigger) {
						if (oInterferingTransition.getGuard() == null
								|| oInterferingTransition.getGuard()
										.getSpecification() == null) {
							for (Trigger oInterferingTrigger : oInterferingTransition
									.getTriggers()) {
								colEvents
										.remove(oInterferingTrigger.getEvent());
							}
						} else {
							// conjunct the negated guard of the super
							// transition
							String sGuard = SystemModelHelper
									.getTransitionGuard(oInterferingTransition);
							// if guard of super transition is "else" -> resolve
							// it!
							if (sGuard.equals("else")) {
					...
					// there were events previously - now, they are removed ->
					// mark transition!
				...
				// should transitions be splitted so that each has at most one
				// event?
				if (Configuration.getSplitUpTransitionsToOneEventLimit()
					...
					for (Event oEvent : colEvents) {
						...
						copyTransition(oTransition, colTmpEvents,
								colAdditionalGuardNegations, bAddTransition);
					}
				} else {
					copyTransition(oTransition, colEvents,
							colAdditionalGuardNegations, bAddTransition);
				}
			}
		}
	...
	private void copyTransition(Transition in_oTransition,
		...
		TCGTransition oTCGTransition = m_oTCGHelper.addTransition(
				m_colActivityNode2TCGNode.get(in_oTransition.getSource()),
				m_colActivityNode2TCGNode.get(in_oTransition.getTarget()));
		...
		if (!in_bAddTransition) {
			...
			m_oTCGHelper
					...
					.appendln(
							"   from "
									+ oTCGTransition.getSourceNode().getName());
			m_oTCGHelper
					.getLogger()
					.getFormattedOutput()
					.appendln(
							"   to   "
									+ oTCGTransition.getTargetNode().getName());
			m_oTCGHelper.getLogger().getFormattedOutput()
					.appendln("   triggered by events:");
			for (TCGEvent oEvent : oTCGTransition.getEvents()) {
				m_oTCGHelper.getLogger().getFormattedOutput()
						.appendln("      " + oEvent.getName());
			}
		}
	}
	...
	/*
	 * adds event information and a default event if the source node is a State
	 * Node
	 */
	private void addEvents(Transition in_oTransition,
			TCGTransition in_oActualTCGTransition, List<Event> in_colEvents) {
		// determine the effect of the transition - select their parameters for
		// event
		Operation oTransitionEffect = null;
		if (in_oTransition.getEffect() != null
				&& in_oTransition.getEffect() instanceof FunctionBehavior) {
			BehavioralFeature oFeature = ((FunctionBehavior) in_oTransition
					.getEffect()).getSpecification();
		...
		// impact e.g. on conflict checks for test goal generation -> may result
		// in multiple definitions of the same parameter
		...
		if (in_oTransition.getSource() instanceof State) {
			...
			if (in_colEvents.isEmpty()) {
				if (bUsedUniqueEventInstances)
					oUsedEvent = m_colEventMapping.get(null);
				if (oUsedEvent == null) {
					oUsedEvent = TCGEventHelperClass.createTCGEvent(null, true);
					if (bUsedUniqueEventInstances)
						m_colEventMapping.put(null, oUsedEvent);
				}
				...
				for (Event oEvent : in_colEvents) {
					if (bUsedUniqueEventInstances)
						oUsedEvent = m_colEventMapping.get(oEvent);
					if (oUsedEvent == null) {
						oUsedEvent = TCGEventHelperClass.createTCGEvent(oEvent,
								true);
						if (bUsedUniqueEventInstances)
							m_colEventMapping.put(oEvent, oUsedEvent);
					}
					in_oActualTCGTransition.getEvents().add(oUsedEvent);
					TCGEventHelperClass.createMatchingForAllParameters(
							oTransitionEffect, oUsedEvent,
	...
	private void addTransitionGuard(Transition in_oTransition,
		...
		Namespace oNamespace = SystemModelHelper
				.getNamespaceForEvent((in_oTransition.getSource()
						.getContainer() != null ? in_oTransition.getSource()
						: in_oTransition.getTarget()), TCGEventHelperClass
						.getEventOfTCGEvent(oEvent), in_oTransition);
		org.eclipse.uml2.uml.Class oContextClass = Configuration
				.getContextClassForVertex((in_oTransition.getSource()
						.getContainer() != null ? in_oTransition.getSource()
		...
		in_oActualTCGTransition
				.setPrecondition(TCGDisjunctiveNormalFormHelperClass
						.getDNFForCondition(sGuard, oNamespace, oContextClass,
								Configuration.getOCLGuardId(),
								in_bSplitInequationsBeforeTestGoalDefinition));
		if (bIsElseGuard) {
			// mark the else-guard so that no complex test goals are created for
			// it
			in_oActualTCGTransition.getPrecondition().setUseForTestGoals(
					Configuration.getUseElseForTestGoals());
		}

		// add information if transition can be active at all (e.g. if there is
		// a contradicting guard)
		if (TCGDisjunctiveNormalFormHelperClass
				.allConjunctionsRemovedBecauseOfInfeasibility(in_oActualTCGTransition
						.getPrecondition())) {
			in_oActualTCGTransition.setCanBeProcessed(false);
		}
	}

	private String createElseGuardCondition(Transition in_oTransition)
			throws Exception {
		...
		for (Transition oOutgoingTransition : oSourceState.getOutgoings()) {
			...
			// only consider transitions that are not the current one
			// (in_oTransition)
			if (oOutgoingTransition.equals(in_oTransition)) {
				continue;
			}

			// transition has to have the same trigger as the original
			// transition!
			if (oOutgoingTransition.getTriggers().isEmpty()) {
				...
				if (in_oTransition.getTriggers().isEmpty()) {
					...
					for (Trigger oOutgoingTrigger : oOutgoingTransition
							.getTriggers()) {
						for (Trigger oInTrigger : in_oTransition.getTriggers()) {
							if (oOutgoingTrigger.getEvent().equals(
									oInTrigger.getEvent()))
								bOverlap = true;
						}
					}
			...
			String sOutgoingGuard = ((LiteralString) oOutgoingTransition
					.getGuard().getSpecification()).getValue();
			if (sOutgoingGuard.compareTo("else") == 0) {
				throw new Exception("too many else conditions from node >"
						+ oSourceState.getQualifiedName() + "<");
			} else if (sOutgoingGuard.isEmpty()) {
				throw new Exception(
						"one empty guard and one else guard together are not allowed from node >"
								+ oSourceState.getQualifiedName() + "<");
			}

			// negate all other guards of transitions triggered by the same
			// event and conjoin them at the else-guard
	...
	private void addStateInvariant(State in_oOriginalState,
			TCGRealNode in_oTCGNode,
			boolean in_bSplitInequationsBeforeTestGoalDefinition) {
		// guard condition
		String sGuard = SystemModelHelper.getStateInvariant(in_oOriginalState);
		// corresponding namespace
		org.eclipse.uml2.uml.Class oNamespace = Configuration
				.getContextClassForVertex(in_oOriginalState);

		in_oTCGNode.setCondition(TCGDisjunctiveNormalFormHelperClass
				.getDNFForCondition(sGuard, oNamespace, oNamespace,
						Configuration.getOCLGuardId(),
						in_bSplitInequationsBeforeTestGoalDefinition));
	}

	private void addEffectPreAndPostcondition(Transition in_oTransition,
			TCGTransition in_oActualTCGTransition,
			boolean in_bSplitInequationsBeforeTestGoalDefinition)
			throws Exception {
		Operation oBehavior = TestCaseGraphHelper
				.getContextEffectSpecification(in_oTransition);
		org.eclipse.uml2.uml.Class oContextClass = Configuration
				.getContextClassForVertex((in_oTransition.getSource()
						.getContainer() != null ? in_oTransition.getSource()
						: in_oTransition.getTarget()));
		...
		if (oBehavior != null) {
			...
			if (!in_oActualTCGTransition.getEvents().isEmpty()
				...
				in_oActualTCGTransition = m_oTCGHelper
						.insertTransitionBetweenTransitionAndTargetNode(in_oActualTCGTransition);
				in_oActualTCGTransition.getSourceNode().setName(
			...
			if (oBehavior.getPreconditions() != null) {
				...
				if (!oBehavior.getPreconditions().isEmpty()) {
					Constraint oPreConstraint = oBehavior.getPreconditions()
							.get(0);
					String sPreExpression = SystemModelHelper
							.getConstraintSpecification(oPreConstraint);
					in_oActualTCGTransition
							.setPrecondition(TCGDisjunctiveNormalFormHelperClass
									.getDNFForCondition(sPreExpression,
											oBehavior, oContextClass,
			...
			if (oBehavior.getPostconditions() != null) {
				if (!oBehavior.getPostconditions().isEmpty()) {
					Constraint oPostConstraint = oBehavior.getPostconditions()
							.get(0);
					String sPostExpression = SystemModelHelper
							.getConstraintSpecification(oPostConstraint);
					in_oActualTCGTransition
							.setPostcondition(TCGDisjunctiveNormalFormHelperClass
									.getDNFForCondition(sPostExpression,
											oBehavior, oContextClass,
