 ...
 * 
 */
package net.sf.parteg.input.statemachine.handlers;
 ...
 * 
 */
public class CreateTestHandler extends AbstractHandler implements IHandler {
	...
	private StateMachine prepareTestGeneration() {
		...
		if (sel instanceof IStructuredSelection) {
			...
			if (oSelection.getFirstElement() instanceof StateMachine) {
				oStateMachine = (StateMachine) (oSelection.getFirstElement());
			}
			// state machine from a state machine diagram file
				...
				oStateMachine = getReferencedStateMachineFromDiagram(oFile);
			} else {
		...
		URI oFileURI = UMLStateMachineHelperClass
				.getPackageOfStateMachine(oStateMachine).eResource().getURI();
